...
# 1. 完了済みタスクだけを表示する関数 show_done_tasks() を作る
#    - todo_list の中から done が True のタスクだけ表示する
#    - 表示フォーマットは show_tasks() と同じでOK

# 2. 未完了タスクだけを表示する関数 show_pending_tasks() を作る
#    - todo_list の中から done が False のタスクだけ表示する
#    - 表示フォーマットは show_tasks() と同じでOK

# 3. キーワードでタスクを検索する関数 search_tasks(keyword) を作る
#    - keyword を含むタスク名だけ表示する
#    - 期限や完了フラグも合わせて表示
#    - 見つからなければ「該当するタスクはありません」と表示
...
# - 未完了タスクだけ表示
# - キーワード検索でヒットするものだけ表示
# - キーワード検索でヒットしない場合のメッセージも確認

# --- 課題⑩ ---
# タスクをファイルに保存・読み込みできるようにしよう
#
# 1. save_tasks(filename) という関数を作る
#    - todo_list を JSON 形式でファイルに保存する
#
...
# --- 課題⑪ ---
# タスクを整理するために「ソート機能」を追加しよう。
#
# 1. 期限順に並べる機能
#    - sort_tasks_by_deadline() を作る
#    - タスクを期限が早い順に並べて表示する
#    - 表示フォーマットは show_tasks() と同じでOK
#
# 2. 名前順に並べる機能
#    - sort_tasks_by_name() を作る
#    - タスク名を五十音順（アルファベットなら辞書順）に並べて表示する
#
# 3. 余裕があれば：
#    - 完了済みタスクを最後に回すオプションを付けても良い
#
# --- 動作確認例 ---
# add_task("読書", "2025-08-21")
# add_task("買い物", "2025-08-19")
# add_task("ゲーム", "2025-08-20")
#
